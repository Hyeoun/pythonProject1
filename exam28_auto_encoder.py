...
Original file is located at
    https://colab.research.google.com/drive/1A7bqFsf7GL1LFhC684yVs9FUfgYGhUMB
"""
...
(x_train, _), (x_test, _) = mnist.load_data()  # 손글씨 이미지 다운
...
for i in range(n):
  ax = plt.subplot(2, 10, i + 1)  # 1행부터 원본 출력
  plt.imshow(x_test[i])
  ax.get_xaxis().set_visible(False)  # x축 선이 안그려짐
  ax.get_yaxis().set_visible(False)

  ax = plt.subplot(2, 10, i + 1 + n)  # 2행부터 모델 돌린거 출력
...
x = MaxPool2D((2, 2), padding='same')(x)  # 이미지를 2*2분의 1수준으로 줄인다.
...
x = UpSampling2D((2, 2))(x)  # 이미지를 2*2수준으로 늘린다.
x = Conv2D(8, (3, 3), activation='relu', padding='same')(x)
x = UpSampling2D((2, 2))(x)
x = Conv2D(16, (3, 3), activation='relu')(x)  # 이 부분에서는 이미지를 2 줄인다.
x = UpSampling2D((2, 2))(x)
...
noise_facter = 0.5
x_train_noisy = conv_x_train + noise_facter * np.random.normal(loc=0.0, scale=1.0, size=conv_x_train.shape)
x_test_noisy = conv_x_test + noise_facter * np.random.normal(loc=0.0, scale=1.0, size=conv_x_test.shape)
x_train_noisy = np.clip(x_train_noisy, 0.0, 1.0)  # 0.0보다 작으면 그냥 0.0으로, 1.0보다 크면 그냥 1.0으로 맞춘다.
